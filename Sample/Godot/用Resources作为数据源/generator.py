...
def gen(data, output):
    ...
    [ext_resource path="{data_class_url}" type="Script" id=1]
    ...
    for row in data["table"]:
        ...
        for index,value in enumerate(row):
            ...
            row_data[field_name] = value or "null"
        table[row_data['id']] = row_data # 我们规定第一个字段是ID字段
    
    table_str = pprint.pformat(table,indent=2).replace("'",'"')
    
    code = template.format(data=table_str, data_class_url=data_class_url)

    code = textwrap.dedent(code)
    
    with open(output+".tres", 'w+') as f:
        f.write(code)
