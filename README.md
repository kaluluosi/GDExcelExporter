...
这是用Excel实现的导出工具，可以把Excel表导出成字典，供自定义加工脚本把这个字典加工成
你需要的数据结构，然后再通过jinja等模板工具生成代码文件保存到输出目录。

工作流程如下：

Eexcel表  --读取->  Excel导出工具 -->加工成Dictionary数据成data字典，输出文件名--传递-->调用custom_generator


[1]--> 如果exporter.toml有配置 custom_generator --> 把字典传递给custom_generator脚本并执行该脚本，这个脚本中把生成的代码输出保存到CFG["settings"]["output"]目录下

[2]--> 如果exporter.toml没有配置 custom_generator -->把字典传递给ExcelExporter默认的生成函数（该函数默认生成json），这个脚本中把生成的代码输出保存到CFG["settings"]["output"]目录下

最终-->如果exporter.toml有配置completed_hook，那么就会把CFG传递给completed_hook脚本执行

如图所示，ExcelExporter其实本身并不替我们决定导出生成的数据格式，也不帮我们保存文件，要生成什么样的数据格式是在用户自己编写的custom_generator里加工生成并保存的。

completed_hook同理，他也没规定导出数据文件后要做什么工作，具体要做什么工作由用户来定义。

不过项目的Sample目录用godot项目的示例用法，你可以拷贝去用。
